#!/usr/bin/env bash
# Deliver a folder of Pinterest screenshots to Telegram as a single ZIP *document*.
#
# CRITICAL: uses sendDocument, never sendPhoto. Telegram's photo path re-encodes to
# JPEG and caps the long edge (~1280px), which destroys the board legibility the
...
CAPTION="${2:-Pinterest board capture ($shots shots)}"
