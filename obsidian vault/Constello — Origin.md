...
1. **No engagement loop** — no dopamine mechanism, no metric that goes up. Can't A/B test warmth. Unfundable by conventional VC logic.
2. **Cold start** — solved by the matching layer above.
3. **Anti-commercial by nature** — these relationships exist above legible identity. You can't profile them, can't sell to them.
4. **It doesn't need to scale to be valuable** — structurally unfundable by conventional VC logic, but a kid with a MacBook can build it now. The cold start, not the funding model, was the real challenge.
