...
Free-to-massive-then-cosmetic, the Discord/Fortnite/Valorant model. Core free forever; revenue from the non-essential. Cosmetic monetization fits Constello more naturally than it fits Discord — profile and collection presentation (frames, themes, layouts, custom typography, soundtracks) is *aesthetic identity expression*, which is the product itself, not a skinner-box bolt-on. "Constello should be free" is coherent with this rather than in tension with it: freemium is the model built for giving the core away.

The caveat is what this model *doesn't* solve. Cosmetic revenue is a scale-stage engine — it earns ~nothing until there's a large, engaged base, so it answers "how does this become a large business," not "how is the build funded to get there." The years-0-to-2 runway gap is a separate, unsolved problem (see Token, below).

Cost structure is the other half of "free." The dominant cost is the generative interpretation layer (see Compute economics), so the lever that makes free affordable is attacking inference cost, not adding revenue. BYO-inference — users bringing their own model access — offloads that COGS. The clean form is bring-your-own-API-key; pointing a personal consumer subscription at the app is ToS-risky and fragile and shouldn't be built on. Most users won't bring a key, so inference for the default free path still has to be budgeted — BYO is a power-user relief valve, not the whole answer.

### Token (contested)

Original intent: a Solana launch as initial funding and momentum vehicle, reasoning that crypto markets reward depth of conviction expressed through capital — closer to futarchy than to social media — and that this rhymes with the product thesis (both make conviction legible without engagement performance). Sequencing was load-bearing: ship a closed beta first so the experience is the proof of concept that justifies the token; otherwise the token becomes the product and holders fight any decision that doesn't serve price. Token utility should reinforce the thesis, not undermine it — possibly *fewer* algorithmic privileges for holders, not more (the audiophile model: people who care about the actual signal pay for less processing).

On examination the token does two jobs that come apart. The *movement* job — rally, identity, something to talk about — is real but is produced by the philosophy and product, not the coin; a price chart taxes exactly that, collapsing thesis-discourse toward price and gating identity by P&L (membership that walks out on a drawdown). Better served by the cosmetic/identity economy, which captures the same belonging energy positive-sum. The *funding* job is the runway gap above, and a memecoin fills it badly: with no cashflow right and no in-product utility, the token is not a claim on Constello's value, only on sentiment about it — a successful Constello's value accrues to equity while the token captures only assigned narrative, unanchored in either direction. Binding token-to-value needs either cashflow rights (→ a security, effectively tokenized equity) or in-product utility (the token as rails); neither is currently invoked. And a token belongs to the speculation economy, not the attention economy Constello targets — a pure meme adopts that system's reflexive pricing rather than sublating it.

The honorable core of the crypto impulse survives the critique: permissionless, liquid capital for a vision the gatekeepers can't price (no comps for a first-of-its-kind). That points to *outcome-anchored* instruments — futarchy, revenue-linked claims — the part of crypto worth keeping, not the memecoin, which is its seductive hollow form. Open: a years-0-to-2 funding path that doesn't weld a speculative asset to a sincerity product's identity (angel / grant / bootstrap / earlier revenue), and — if a token at all — the anchored version. Ties to the **Token utility specifics** open thread.
